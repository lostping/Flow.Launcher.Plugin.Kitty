    ...
    public class Paths
    {
        ...
        public static string ReturnPluginSettingsFolder(PluginInitContext context)
        {
            if(IsPortable(context))
            {
                ...
                return Path.Combine(userData, "Settings", "Plugins", "Flow.Launcher.Plugin.Kitty");
            } else
            {
                string appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
                return Path.Combine(appData, "FlowLauncher", "Settings", "Plugins", "Flow.Launcher.Plugin.Kitty");
            }
        }
    }
}
